// =============================================================================
// TikStream AI — Trend Engine Seed Script
//
// 独立运行：使用 TrendEngine 算法引擎为所有产品生成趋势分析快照
// 直接写入数据库，现有 API 即可读取
//
// 用法:
//   $env:DATABASE_URL="postgresql://tikstream:tikstream_password@127.0.0.1:15432/tikstream_ai?schema=public"
//   npx tsx prisma/seed-trends.ts
// =============================================================================
...
// =============================================================================
// Types (mirrored from algorithms/types.ts to avoid NestJS dependency)
// =============================================================================

type TrendLifecycleStage = 'emerging' | 'rising' | 'peak' | 'declining' | 'dying';
type TrendType = 'hashtag' | 'sound' | 'effect' | 'topic';
...
const WEIGHTS = {
  heat: { volume: 0.30, velocity: 0.25, engagement: 0.20, recency: 0.15, creator: 0.10 },
  matching: { categoryAffinity: 0.45, keywordSimilarity: 0.35, audienceOverlap: 0.20 },
  opportunity: { heat: 0.35, match: 0.40, timing: 0.25 },
};

const CATEGORY_AFFINITY_MATRIX: Record<string, Record<string, number>> = {
  beauty: { beauty: 1.0, fitness: 0.2, food: 0.1, tech: 0.1, home: 0.3, pet: 0.1, fashion: 0.8, health: 0.6, lifestyle: 0.5, entertainment: 0.3 },
  fitness: { beauty: 0.3, fitness: 1.0, food: 0.4, tech: 0.3, home: 0.2, pet: 0.1, fashion: 0.3, health: 0.8, lifestyle: 0.6, entertainment: 0.2 },
  food: { beauty: 0.1, fitness: 0.3, food: 1.0, tech: 0.1, home: 0.4, pet: 0.1, fashion: 0.2, health: 0.5, lifestyle: 0.6, entertainment: 0.4 },
  tech: { beauty: 0.1, fitness: 0.4, food: 0.1, tech: 1.0, home: 0.5, pet: 0.2, fashion: 0.2, health: 0.3, lifestyle: 0.4, entertainment: 0.5 },
  home: { beauty: 0.2, fitness: 0.1, food: 0.3, tech: 0.3, home: 1.0, pet: 0.3, fashion: 0.3, health: 0.2, lifestyle: 0.7, entertainment: 0.2 },
  pet: { beauty: 0.1, fitness: 0.1, food: 0.3, tech: 0.1, home: 0.3, pet: 1.0, fashion: 0.1, health: 0.2, lifestyle: 0.5, entertainment: 0.3 },
};

// =============================================================================
// Trend Mock Data (same as TrendMockDataService but standalone)
// =============================================================================

interface MockTrend {
  name: string; type: TrendType; url: string;
  categories: string[]; keywords: string[]; audienceTags: string[];
  lifecycleStage: TrendLifecycleStage; baseHeat: number;
}

const GLOBAL_TRENDS: MockTrend[] = [
  // =========================================================================
  // BEAUTY (美妆护肤) — 15 条
  // =========================================================================
  ...
  // =========================================================================
  // E-COMMERCE & CROSS-CATEGORY (电商+跨品类) — 20 条
  // =========================================================================
...
// =============================================================================
// Algorithm: Heat Scoring
// =============================================================================

function calcVolumeScore(mentions: number): number {
  const MAX = 1_000_000;
  if (mentions <= 0) return 0;
  return Math.min(100, (Math.log10(1 + mentions) / Math.log10(1 + MAX)) * 100);
}

function calcVelocityScore(mentions24h: number, mentions7d: number): number {
  ...
  return Math.max(0, Math.min(100, 50 + 50 * Math.tanh(10 * growthRate)));
}

function calcEngagementScore(likes: number, shares: number, comments: number, videos: number): number {
  if (videos <= 0) return 0;
  const weighted = likes * 1 + shares * 2 + comments * 3;
  const rate = weighted / videos;
  const MAX = 5000;
  return Math.min(100, (Math.log10(1 + rate) / Math.log10(1 + MAX)) * 100);
}

function calcRecencyScore(timestamp: Date): number {
  const ageDays = (Date.now() - timestamp.getTime()) / (24 * 60 * 60 * 1000);
  if (ageDays < 0) return 100;
  if (ageDays > 30) return 0;
  return Math.max(0, 100 * Math.exp((-Math.LN2 * ageDays) / 7));
}

function calcCreatorScore(rate: number): number {
  return Math.min(100, 100 * Math.sqrt(Math.max(0, Math.min(1, rate))));
}

function scoreTrend(dp: TrendDataPoint): TrendHeatResult {
  const w = WEIGHTS.heat;
  const vs = calcVolumeScore(dp.mentionCount7d);
  const vls = calcVelocityScore(dp.mentionCount24h, dp.mentionCount7d);
  const es = calcEngagementScore(dp.likeCount, dp.shareCount, dp.commentCount, dp.videoCount);
  const rs = calcRecencyScore(dp.timestamp);
  const cs = calcCreatorScore(dp.creatorAdoptionRate);
  const heat = Math.max(0, Math.min(100, w.volume * vs + w.velocity * vls + w.engagement * es + w.recency * rs + w.creator * cs));
  return { name: dp.name, type: dp.type, url: dp.url, heatScore: Math.round(heat * 100) / 100, volumeScore: Math.round(vs * 100) / 100, velocityScore: Math.round(vls * 100) / 100, engagementScore: Math.round(es * 100) / 100, recencyScore: Math.round(rs * 100) / 100, creatorScore: Math.round(cs * 100) / 100 };
}

// =============================================================================
// Algorithm: Product Matching
// =============================================================================

function calcCategoryAffinity(trendCategories: string[], productCategory: string): number {
  if (!trendCategories.length) return 10;
  const row = CATEGORY_AFFINITY_MATRIX[productCategory];
  if (!row) {
    let t = 0;
    for (const c of trendCategories) t += CATEGORY_AFFINITY_MATRIX[c]?.[productCategory] ?? 0.15;
    return (t / trendCategories.length) * 100;
  }
  let t = 0;
  for (const c of trendCategories) t += row[c] ?? 0.15;
  return (t / trendCategories.length) * 100;
}

function calcKeywordSimilarity(trendKw: string[], prodKw: string[]): number {
  if (!trendKw.length || !prodKw.length) return 0;
  ...
  const buildTF = (kws: string[]): Map<string, number> => {
    const tf = new Map<string, number>();
    for (const kw of kws) for (const t of tokenize(kw)) tf.set(t, (tf.get(t) || 0) + 1);
    const total = kws.length || 1;
    for (const [k, v] of tf) tf.set(k, v / total);
    return tf;
  };
  const tTF = buildTF(trendKw), pTF = buildTF(prodKw);
  const all = new Set([...tTF.keys(), ...pTF.keys()]);
  let dot = 0, tn = 0, pn = 0;
  for (const token of all) { const a = tTF.get(token) || 0; const b = pTF.get(token) || 0; dot += a * b; tn += a * a; pn += b * b; }
  if (tn === 0 || pn === 0) return 0;
  return Math.max(0, Math.min(100, (dot / (Math.sqrt(tn) * Math.sqrt(pn))) * 100));
}

function calcAudienceOverlap(trendAud: string[], prodAud: string[]): number {
  if (!trendAud.length || !prodAud.length) return 0;
  const ts = new Set(trendAud.map(t => t.toLowerCase()));
  const ps = new Set(prodAud.map(t => t.toLowerCase()));
  let int = 0;
  for (const t of ts) if (ps.has(t)) int++;
  const union = ts.size + ps.size - int;
  return union === 0 ? 0 : (int / union) * 100;
}

function matchTrend(trend: TrendDataPoint, product: ProductInfo): ProductMatchResult {
  const w = WEIGHTS.matching;
  const ca = calcCategoryAffinity(trend.categories, product.category);
  const ks = calcKeywordSimilarity(trend.keywords, [...product.sellingPoints, product.name, ...product.scenarioTags]);
  const ao = calcAudienceOverlap(trend.audienceTags, product.audienceTags);
  const score = Math.max(0, Math.min(100, w.categoryAffinity * ca + w.keywordSimilarity * ks + w.audienceOverlap * ao));
  return { trendName: trend.name, trendType: trend.type, matchScore: Math.round(score * 100) / 100, categoryAffinity: Math.round(ca * 100) / 100, keywordSimilarity: Math.round(ks * 100) / 100, audienceOverlap: Math.round(ao * 100) / 100 };
}

// =============================================================================
// Algorithm: Velocity & Lifecycle
// =============================================================================

function classifyStage(heat: number, velocity: number, accel: number): TrendLifecycleStage {
  const HL = 30, HH = 70, VZ = 0.5, AZ = 0.1;
  const av = Math.abs(velocity);
  if (heat >= HH && av < VZ) return 'peak';
  if (heat >= HH && velocity < -VZ) return 'declining';
  if (heat >= HH && velocity > VZ) return 'rising';
  if (heat >= HL && heat < HH) {
    if (velocity > VZ) return 'rising';
    if (velocity < -VZ) return 'declining';
    return 'peak';
  }
  if (heat < HL) {
    if (velocity > VZ && accel > AZ) return 'emerging';
    if (velocity > VZ) return 'rising';
    if (velocity < -VZ) return 'dying';
    return 'emerging';
  }
  return 'emerging';
}

function estDaysToPeak(stage: TrendLifecycleStage, currentHeat: number, velocity: number): number {
  const PEAK = 95;
  switch (stage) {
    case 'emerging': case 'rising': {
      if (velocity <= 0) return 7;
      const decay = 0.1, rem = PEAK - currentHeat, maxInc = velocity / decay;
      if (rem >= maxInc) return Math.ceil(rem / Math.max(velocity, 0.1));
      return Math.ceil(-Math.log(1 - rem / maxInc) / decay);
    }
    case 'peak': return 0;
...
function estRemainingDays(stage: TrendLifecycleStage, velocity: number, heat: number): number {
  const MIN = 10;
  switch (stage) {
    case 'emerging': return 10 + Math.floor(Math.random() * 20);
    case 'rising': return 7 + Math.floor(Math.random() * 14);
    case 'peak': {
      if (velocity >= 0) return 5;
      const dr = Math.abs(velocity);
      return dr <= 0 ? 7 : Math.max(1, Math.ceil((heat - MIN) / dr));
    }
    case 'declining': return Math.max(1, Math.ceil((heat - MIN) / Math.max(Math.abs(velocity), 0.5)));
    case 'dying': return 1 + Math.floor(Math.random() * 3);
    default: return 7;
  }
}

function analyzeVelocity(trendName: string, trendType: TrendType, stage: TrendLifecycleStage, heat: number, baseHeat: number): TrendVelocityResult {
  // Simulate velocity based on lifecycle stage
  const stageVelocities: Record<TrendLifecycleStage, number> = { emerging: 3.5, rising: 1.5, peak: 0.1, declining: -2.0, dying: -5.0 };
  const stageAccel: Record<TrendLifecycleStage, number> = { emerging: 0.3, rising: 0.1, peak: -0.05, declining: -0.2, dying: -0.1 };
  const v = stageVelocities[stage] * (heat / baseHeat) * (0.8 + Math.random() * 0.4);
  const a = stageAccel[stage] * (0.8 + Math.random() * 0.4);
  const dtp = estDaysToPeak(stage, heat, v);
  const rd = estRemainingDays(stage, v, heat);
  const conf = Math.min(100, 40 + Math.random() * 30);
  return { trendName, trendType, currentHeat: heat, velocity: Math.round(v * 1000) / 1000, acceleration: Math.round(a * 1000) / 1000, lifecycleStage: stage, daysToPeak: Math.round(dtp), remainingDays: Math.max(0, Math.round(rd)), confidence: Math.round(conf * 100) / 100 };
}

// =============================================================================
// Algorithm: Opportunity Ranking
// =============================================================================

function calcTimingScore(stage: TrendLifecycleStage, daysToPeak: number): number {
  switch (stage) {
    case 'emerging': return daysToPeak >= 5 ? 95 : daysToPeak >= 3 ? 85 : 75;
    case 'rising': return daysToPeak >= 3 ? 70 : daysToPeak >= 1 ? 55 : 45;
    case 'peak': return 40;
...
function determineAction(opp: number, heat: number, match: number, stage: TrendLifecycleStage): string {
  if (opp >= 70 && heat >= 60 && match >= 50) return 'jump_in_immediately';
  if (opp >= 60 && heat >= 50 && (stage === 'emerging' || stage === 'rising')) return 'prepare_content';
  if (opp >= 65 && match >= 40) return 'prepare_content';
  if (opp >= 40 && (stage === 'emerging' || stage === 'rising')) return 'monitor_closely';
  if (opp >= 30 && match >= 40) return 'cautious_test';
  if (opp >= 20) return 'wait_and_see';
  return 'avoid';
}

function estReach(heat: number, match: number, stage: TrendLifecycleStage): number {
  const BASE = 10000;
  const sf: Record<TrendLifecycleStage, number> = { emerging: 0.3, rising: 0.8, peak: 1.0, declining: 0.5, dying: 0.1 };
  return Math.round((BASE * Math.max(0.1, heat / 50) * Math.max(0.1, match / 50) * (sf[stage] || 0.5)) / 1000) * 1000;
}

function genTips(name: string, type: TrendType, action: string, stage: TrendLifecycleStage): string[] {
  const actionTips: Record<string, string[]> = {
    jump_in_immediately: [`立即围绕「${name}」创作内容，抢占流量窗口`, `使用趋势相关标签和音效，最大化曝光`, `准备多版素材进行 A/B 测试，快速迭代`],
    prepare_content: [`开始策划与「${name}」相关的内容脚本`, `调研对标账号在类似趋势下的表现数据`, `准备 3-5 版不同风格的素材预案`],
    monitor_closely: [`持续监测「${name}」的热度变化和竞争态势`, `关注同类商品在趋势下的转化数据`, `提前储备相关素材和脚本模板`],
    cautious_test: [`低预算测试「${name}」与商品的结合效果`, `重点观察 CTR 和转化率指标`, `控制投放预算，验证 ROI 后决定是否加码`],
    wait_and_see: [`将「${name}」加入观察列表`, `关注趋势演变方向和相关子趋势`, `优化自身商品卖点，等待更好的匹配时机`],
    avoid: [`不建议跟随「${name}」，趋势匹配度过低`, `将资源投入到与商品更匹配的其他趋势`, `关注是否有新兴替代趋势出现`],
  };
  const tips = [...(actionTips[action] || actionTips.wait_and_see)];
  if (type === 'hashtag') tips.push(`在视频标题和描述中合理使用 #${name.replace(/^#/, '')} 标签`);
  if (type === 'sound') tips.push(`在视频中使用「${name}」作为背景音乐`);
  if (stage === 'emerging') tips.push('趋势处于萌芽期，早期进入可获得最大流量红利');
  if (stage === 'peak') tips.push('趋势已到高峰期，建议差异化切入而非简单跟风');
...
function heatToMentions(heat: number, days: number): number {
  const daily = Math.pow(10, 1 + (heat / 100) * 5) * (0.8 + Math.random() * 0.4);
  return daily * days;
}

function stageVeloMul(stage: TrendLifecycleStage): number {
  const m: Record<TrendLifecycleStage, number> = { emerging: 0.3, rising: 0.15, peak: 0.02, declining: -0.1, dying: -0.3 };
  return m[stage] || 0;
}

function heatToAdoption(heat: number, stage: TrendLifecycleStage): number {
  const sm: Record<TrendLifecycleStage, number> = { emerging: 0.5, rising: 0.8, peak: 1.0, declining: 0.6, dying: 0.2 };
  return Math.max(0, Math.min(1, (heat / 100) * 0.5 * (sm[stage] || 1)));
}

function trendToDataPoint(t: MockTrend): TrendDataPoint {
  const m7 = heatToMentions(t.baseHeat, 7);
  const m24 = heatToMentions(t.baseHeat, 1) * (1 + stageVeloMul(t.lifecycleStage));
  const vc = Math.round(m7 * 0.05);
  const lc = Math.round(m7 * (20 + Math.random() * 30));
  return {
    name: t.name, type: t.type, url: t.url || undefined,
    mentionCount24h: Math.round(m24), mentionCount7d: Math.round(m7),
    likeCount: Math.round(lc), shareCount: Math.round(lc * (0.05 + Math.random() * 0.1)),
    commentCount: Math.round(lc * (0.03 + Math.random() * 0.07)), videoCount: Math.round(vc),
    creatorAdoptionRate: heatToAdoption(t.baseHeat, t.lifecycleStage),
    categories: t.categories, keywords: t.keywords, audienceTags: t.audienceTags, timestamp: new Date(),
  };
}

function getDefaultSellingPoints(cat: string): string[] {
  ...
  return d[cat] || ['品质好', '服务好', '价格优'];
}

function getDefaultAudience(cat: string): string {
  const d: Record<string, string> = {
    beauty: '18-35岁女性', fitness: '20-40岁健身爱好者', food: '18-45岁美食爱好者',
    tech: '18-40岁数码爱好者', home: '20-40岁家居爱好者', pet: '20-45岁养宠人群',
  };
  return d[cat] || '全年龄段消费者';
}

function getDefaultAudienceTags(cat: string): string[] {
  const d: Record<string, string[]> = {
  ...
  return d[cat] || ['Z世代', '网购族'];
}

function getDefaultScenarioTags(cat: string): string[] {
  const d: Record<string, string[]> = {
    beauty: ['daily', 'skincare', 'makeup'], fitness: ['workout', 'daily', 'outdoor'],
    food: ['daily', 'party', 'home'], tech: ['daily', 'work', 'commute'],
    home: ['daily', 'relax', 'decorate'], pet: ['daily', 'home', 'indoor'],
  };
  return d[cat] || ['daily'];
}
...
async function main(): Promise<void> {
  console.log('========================================');
  console.log('Trend Engine Seed — Algorithm-Generated Trend Snapshots');
  console.log('========================================\n');

  // 1. Clean old data
  console.log('[1] Cleaning old trend snapshots...');
  await prisma.trendSnapshot.deleteMany();
  console.log('  Done.\n');

  // 2. Read products
  console.log('[2] Reading products...');
  const products = await prisma.product.findMany();
  if (products.length === 0) {
    console.log('  No products found. Run comprehensive-seed first.');
    return;
  }
  console.log(`  Found ${products.length} products.\n`);

  // 3. For each product, run algorithm engine
  console.log('[3] Running Trend Engine for each product...\n');

  const categoryTrends = new Map<string, MockTrend[]>();
  for (const t of GLOBAL_TRENDS) {
    for (const c of t.categories) {
      if (!categoryTrends.has(c)) categoryTrends.set(c, []);
      categoryTrends.get(c)!.push(t);
    }
  }
  ...
  for (const product of products) {
    ...
    const sellingPoints = (product.sellingPoints as string[]) || getDefaultSellingPoints(cat);

    // Get trends relevant to this category
    const catSpecific = categoryTrends.get(cat) || [];
    const global = GLOBAL_TRENDS.filter(t => !catSpecific.includes(t));
    const seen = new Set<string>();
    const relevant = [...catSpecific.sort((a, b) => b.baseHeat - a.baseHeat), ...global.sort((a, b) => b.baseHeat - a.baseHeat)]
      .filter(t => { if (seen.has(t.name)) return false; seen.add(t.name); return true; })
      .slice(0, 20);

    // Convert to data points
    const dataPoints = relevant.map(trendToDataPoint);

    // Build product info
    const productInfo: ProductInfo = {
      productId: product.id, name: productName, category: cat,
      sellingPoints, targetAudience: getDefaultAudience(cat),
      audienceTags: getDefaultAudienceTags(cat), scenarioTags: getDefaultScenarioTags(cat),
    };

    // Phase 1: Heat Scoring
    const heatResults = dataPoints.map(dp => scoreTrend(dp)).sort((a, b) => b.heatScore - a.heatScore);

    // Phase 2: Product Matching
    const matchResults = dataPoints.map(dp => matchTrend(dp, productInfo)).sort((a, b) => b.matchScore - a.matchScore);

    // Phase 3: Velocity Analysis
    const velocityResults = relevant.map(t => analyzeVelocity(t.name, t.type, t.lifecycleStage, t.baseHeat, t.baseHeat));
    ...
    const opportunities: OpportunityResult[] = heatResults
      .map(h => {
        const m = matchMap.get(h.name);
        const v = velocityMap.get(h.name);
        if (!m || !v) return null;
        const ts = calcTimingScore(v.lifecycleStage, v.daysToPeak);
        const opp = Math.max(0, Math.min(100, WEIGHTS.opportunity.heat * h.heatScore + WEIGHTS.opportunity.match * m.matchScore + WEIGHTS.opportunity.timing * ts));
        const action = determineAction(opp, h.heatScore, m.matchScore, v.lifecycleStage);
        const reach = estReach(h.heatScore, m.matchScore, v.lifecycleStage);
        return {
          trendName: h.name, trendType: h.type, url: h.url,
          opportunityScore: Math.round(opp * 100) / 100,
          heatScore: h.heatScore, matchScore: m.matchScore,
          timingScore: Math.round(ts * 100) / 100,
          lifecycleStage: v.lifecycleStage,
          recommendedAction: action, potentialReach: reach,
          adaptationTips: genTips(h.name, h.type, action, v.lifecycleStage),
        };
      })
      .filter((o): o is OpportunityResult => o !== null)
      .sort((a, b) => b.opportunityScore - a.opportunityScore)
      .slice(0, 5);

    // Map to API format
    const trends = heatResults.slice(0, 20).map(h => ({
      type: h.type,
      name: h.name,
      url: h.url || '',
      popularity_score: Math.round(h.heatScore),
      growth_rate: Math.round((velocityMap.get(h.name)?.velocity || 0) / 50 * 100) / 100,
      expiration_days: Math.max(1, velocityMap.get(h.name)?.remainingDays || 7),
    }));

    const recommendations = opportunities.map(o => {
      const t = trends.find(tt => tt.name === o.trendName);
      return {
        trend: t || { type: o.trendType, name: o.trendName, url: o.url || '', popularity_score: Math.round(o.heatScore), growth_rate: 0, expiration_days: 7 },
        product_match_score: Math.round(o.matchScore),
        adaptation_tips: o.adaptationTips.slice(0, 3),
    ...
    await prisma.trendSnapshot.create({
      data: {
        ...
        trendsJson: trends as unknown as Record<string, unknown>,
        recommendationsJson: recommendations as unknown as Record<string, unknown>,
        generatedBy: `ALGORITHM_ENGINE_v2.0`,
        ttlSeconds: 3600,
    ...
    console.log(`  ✓ ${product.skuCode} (${cat}): ${heatResults.length} trends, ${opportunities.length} opportunities | Top: "${topOpp?.trendName}" (opp=${topOpp?.opportunityScore}, ${topOpp?.recommendedAction})`);
  }

  console.log(`\n========================================`);
  console.log(`Done! Created ${totalSnapshots} algorithm-generated trend snapshots.`);
  console.log(`Algorithm Pipeline: Heat Scoring → Product Matching → Velocity Analysis → Opportunity Ranking`);
  console.log(`========================================`);
}
