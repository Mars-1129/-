// =============================================================================
// TikStream AI — Script Template Generate 自动化测试基座
// 对应功能: POST /api/v1/scripts/generate/template (模板驱动剧本生成)
// 对应模块: Script (人员B)
...
const mockScriptShotPayloadFactory = (
  ...
  shot_index: index,
  duration: 3.0,
  scene_description_query: `close-up shot ${index} of product feature guided by template`,
  visual_description: `镜头${index}：按照模板策略展示产品核心功能与使用场景。`,
  camera_movement: 'Static',
  transition_type: index === 1 ? 'Fade_In' : 'Dissolve',
  voiceover_text: `第${index}段旁白：模板策略驱动的产品卖点表达。`,
...
const mockTemplateAIValidResponse = JSON.stringify({
  title: '快节奏卷发棒测评 — 3分钟打造明星同款',
  video_duration: 13.5,
  style_vibe: 'energetic/comparison',
  shots: [
    {
      shot_index: 1,
      duration: 2.5,
      scene_description_query:
        'split screen of messy hair vs perfect curls, strong visual contrast opener',
      visual_description:
        '分屏对比乱发与精致卷发，强烈视觉冲击开场，对标模板 hook_style=visual_contrast 策略。',
      camera_movement: 'Dolly_In_Fast',
      transition_type: 'Fade_In',
      voiceover_text:
        '还在为每天早起弄头发发愁？看看这个无线卷发棒有多神奇！',
      subtitle_text: '早起乱发？看这个神器',
    ...
    },
    {
      ...
      duration: 3.0,
      scene_description_query:
        'product unboxing and quick heating demo on white surface',
      visual_description:
        '白色桌面产品开箱与升温演示，LED温控屏亮起，快速展示3档智能控温功能。',
      camera_movement: 'Dolly_In_Fast',
      transition_type: 'Dissolve',
      voiceover_text:
        '三档智能控温，10分钟快充，陶瓷涂层不伤发质。',
      subtitle_text: '3档控温｜陶瓷涂层',
      safe_zone_bounding_box: [0.1, 0.72, 0.9, 0.9],
    },
    {
      shot_index: 3,
      duration: 3.5,
      scene_description_query:
        'model curling hair in bright studio, showing result after one pass',
      visual_description:
        '模特明亮工作室使用卷发棒造型，慢镜展示一夹成型效果，对标模板中段叙事节奏。',
      camera_movement: 'Pan_Left',
      transition_type: 'Dissolve',
      voiceover_text:
        '32mm大卷设计，一夹成型，比传统卷发棒快三倍还省心。',
      subtitle_text: '32mm大卷｜一夹成型',
      safe_zone_bounding_box: [0.08, 0.7, 0.92, 0.88],
    },
    {
      shot_index: 4,
      duration: 2.5,
      scene_description_query:
        'before after comparison with glowing review quotes overlay',
      visual_description:
        '前后效果对比 + 用户好评浮层叠加，对标模板 social_proof 段落。',
      camera_movement: 'Tilt_Up',
      transition_type: 'Wipe',
      voiceover_text:
        '上千好评，用户都说用完再也回不去传统卷发棒了。',
      subtitle_text: '上千好评｜用户力荐',
    ...
    },
    {
      ...
      duration: 2.0,
      scene_description_query:
        'product beauty shot with key floating specs, end card CTA in last 2 seconds',
      visual_description:
        '360度产品展示+关键参数浮层，最后2秒强CTA收尾，对标模板 cta_placement=last_2_seconds。',
      camera_movement: 'Dolly_Out',
      transition_type: 'Fade_In',
      voiceover_text:
        '限时新客优惠，点击下方链接，马上拥有你的专属造型神器。',
      subtitle_text: '限时优惠｜立即下单',
      safe_zone_bounding_box: [0.1, 0.74, 0.9, 0.92],
    },
  ],
});

const mockTemplateAIResponseTagged = `\`\`\`json\n${mockTemplateAIValidResponse}\n\`\`\``;
...
describe('ScriptTemplateGenerate — 模板驱动剧本生成', () => {
  // ===========================================================================
  // 全局测试上下文
  // ===========================================================================
  ...
  let buildTemplatePrompt: (params: Record<string, unknown>) => {
    systemPrompt: string;
    userPrompt: string;
  };
  ...
  // ---- 模拟 generateTemplate 编排函数 ----
  let generateTemplate: (
    dto: Record<string, unknown>,
    deps: {
      prisma: MockPrismaService;
      doubao: typeof mockDoubaoTextProvider;
      buildPrompt: typeof buildTemplatePrompt;
      ...
      validateTpl: typeof validateTemplate;
    },
  ) => Promise<Record<string, unknown>>;

  beforeAll(() => {
    // ---- 注入 Template Prompt Builder mock ----
    buildTemplatePrompt = (params) => {
      const sellingPoints = (params.selling_points as string[]) || [];
      const styleVibe = (params.style_vibe as string) || 'mixed';
      ...
      const strategySummary = (params.strategy_summary as string) || '';
      const factorJson = (params.factor_json as Record<string, unknown>) || {};

      const factorEntries = Object.entries(factorJson);
      ...
      const sortedFactors: Array<[string, unknown]> = [];
      for (const key of PRIORITY_ORDER) {
        const entry = factorEntries.find(([k]) => k === key);
        if (entry) sortedFactors.push(entry);
      }
      for (const entry of factorEntries) {
        if (!PRIORITY_ORDER.includes(entry[0])) {
          sortedFactors.push(entry);
        }
      }
      const truncatedFactors = sortedFactors.slice(0, 6);
      const truncatedFactorObj = Object.fromEntries(truncatedFactors);

      const systemPrompt = [
        '你是一名专业的 TikTok Shop 短视频脚本创作专家，你正在执行「模板驱动」脚本生成任务。',
        `输出语言: ${(params.language as string) || 'zh-CN'}。`,
        `画面比例: ${aspectRatio}。`,
        '',
        '以下是一个已验证的高转化剧作模板的策略摘要与关键因子配置，你需要严格遵循此模板的叙事结构：',
        `策略摘要: ${strategySummary}`,
        `关键因子: ${JSON.stringify(truncatedFactorObj)}`,
        '',
        '模板生成要求：',
        '1. 严格遵循模板的叙事结构、节奏模式和情感曲线。',
        '2. 使用模板指定的运镜偏好、转场偏好、BGM风格。',
        '3. 将新商品卖点嵌入模板叙事框架中，内容必须100%原创。',
        '4. 遵循模板的CTA放置策略和Hook风格。',
        '',
    ...
    validateTemplate = async (templateId: string, prisma: MockPrismaService) => {
      ...
      if (template.status !== 'ACTIVE') {
        throw Object.assign(new Error(`模板 ${templateId} 状态为 ${template.status}，不可用于生成`), {
          errorCode: 'TEMPLATE_NOT_ACTIVE',
          statusCode: HttpStatus.BAD_REQUEST,
        });
      }

      const factorJson = template.factor_json as Record<string, unknown>;
      if (!factorJson || Object.keys(factorJson).length === 0) {
        throw Object.assign(new Error('模板因子配置为空，无法用于生成'), {
          errorCode: 'TEMPLATE_FACTOR_EMPTY',
          statusCode: HttpStatus.BAD_REQUEST,
        });
      }

      if (!template.strategy_summary || template.strategy_summary.trim().length === 0) {
        throw Object.assign(new Error('模板策略摘要为空，无法用于生成'), {
          errorCode: 'TEMPLATE_FACTOR_EMPTY',
          statusCode: HttpStatus.BAD_REQUEST,
        });
      }
    ...
    // ---- 注入 generateTemplate 编排函数 (Service 层核心逻辑的测试替身) ----
    generateTemplate = async (dto, deps) => {
      const { prisma, doubao, buildPrompt, parseResponse, validateSchema, runCompliance, validateTpl } = deps;
      ...
      const templateId = dto.template_id as string;
      const template = await validateTpl(templateId, prisma);

      const promptParams: Record<string, unknown> = {
        selling_points: dto.selling_points || product.selling_points || [],
        style_vibe: dto.style_vibe || 'mixed',
        ...
        strategy_summary: template.strategy_summary,
        factor_json: template.factor_json,
        title: dto.title,
      };
      ...
      const scriptRecord: TestPersistedScript = {
        ...
        style_vibe: dto.style_vibe as string || 'mixed',
      ...
      try {
        if (typeof prisma.$transaction === 'function') {
          ...
          await prisma.script.create({ data: scriptRecord });
          await prisma.scriptShot.createMany({ data: shotRecords });
        }
      } catch (e) {
  ...
  describe('【正常流】合法输入 → 完整 ScriptGenerateResponse 输出', () => {
    const validRequest = {
      product_id: '00000000-0000-0000-0000-000000000001',
      template_id: '00000000-0000-0000-0000-000000000888',
      title: '快节奏卷发棒测评 — 3分钟打造明星同款',
      language: 'zh-CN',
      selling_points: ['3档智能控温', '10分钟快充', '便携无线', '防烫设计'],
      target_audience: '北美年轻女性,25-35岁',
    ...
    beforeEach(() => {
      ...
      mockDoubao.generateText.mockResolvedValue(mockTemplateAIValidResponse);
    });

    it('TC-STG-001: 模板驱动生成成功 — 返回完整结构', async () => {
      const result = await generateTemplate(validRequest, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
        validateSchema: validateScriptSchema,
        runCompliance: checkCompliance,
        validateTpl: validateTemplate,
      });
      ...
      expect(result.generation_mode).toBe('TEMPLATE_DRIVEN');

      expect(result).toHaveProperty('template_id');
      expect(result.template_id).toBe(validRequest.template_id);
      ...
      expect(result.style_vibe).toBe('energetic/comparison');
    ...
    it('TC-STG-001-EXT: AI 响应含 markdown 代码块包裹时正常解析', async () => {
      mockDoubao.generateText.mockResolvedValue(mockTemplateAIResponseTagged);
    ...
    it('TC-STG-001-EXT: 未传 title 时仍然成功生成', async () => {
      const requestWithoutTitle = { ...validRequest };
      delete requestWithoutTitle.title;

      const result = await generateTemplate(requestWithoutTitle, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
    ...
    it('TC-STG-001-EXT: 未传 target_audience 时仍然成功生成', async () => {
      const requestWithoutAudience = { ...validRequest };
      delete requestWithoutAudience.target_audience;

      mockDoubao.generateText.mockResolvedValue(mockTemplateAIValidResponse);

      const result = await generateTemplate(requestWithoutAudience, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
    ...
    it('TC-STG-001-EXT: 未传 style_vibe 时兜底为 mixed 仍然成功生成 (模板自带风格)', async () => {
      const requestWithoutStyleVibe = { ...validRequest };
      delete requestWithoutStyleVibe.style_vibe;

      mockDoubao.generateText.mockResolvedValue(mockTemplateAIValidResponse);

      const result = await generateTemplate(requestWithoutStyleVibe, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
      ...
      expect(result.shots.length).toBeGreaterThan(0);
      expect(result.generation_mode).toBe('TEMPLATE_DRIVEN');
      expect(result.style_vibe).toBe('mixed');
    });

    it('TC-STG-001-EXT: 未传 selling_points 时使用商品自带卖点仍成功生成', async () => {
      ...
      const result = await generateTemplate(requestWithoutSellingPoints, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
  ...
  describe('【边界流】极端输入 → 系统优雅处理', () => {
    ...
    it('TC-STG-BND-001: selling_points 仅 1 项时成功生成', async () => {
      const request = {
        ...validRequest,
        selling_points: ['单一卖点'],
      };
    ...
    it('TC-STG-BND-002: selling_points 达 10 项时成功生成', async () => {
    ...
    it('TC-STG-BND-003: 总时长恰好 15.0s (临界值) 通过', async () => {
    ...
    it('TC-STG-BND-004: 单分镜时长 1.5s (下限临界值) 通过', async () => {
    ...
    it('TC-STG-BND-005: 单分镜时长 5.0s (上限临界值) 通过', async () => {
    ...
    it('TC-STG-BND-006: aspect_ratio 为 16:9 时正常生成', async () => {
      const request169 = { ...validRequest, aspect_ratio: '16:9' };

      const result = await generateTemplate(request169, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
    ...
    it('TC-STG-BND-007: 所有 CameraMovement 枚举值均可通过校验', async () => {
    ...
    it('TC-STG-BND-008: constraint_list 为空数组时不报错', async () => {
      const requestNoConstraint = { ...validRequest, constraint_list: [] };

      const result = await generateTemplate(requestNoConstraint, {
        prisma: mockPrisma,
        doubao: mockDoubao,
        buildPrompt: buildTemplatePrompt,
        parseResponse: parseScriptFromAIResponse,
    ...
    it('TC-STG-BND-009: safe_zone_bounding_box 边界值 [0,0,1,1] 通过', async () => {
    ...
    it('TC-STG-BND-010: template factorJson 仅含 1 个因子时仍可成功生成', async () => {
      const minimalTemplate = mockTemplateFactory({
        factor_json: { optimal_shot_count: 4 },
      });

      mockPrisma.template.findUnique.mockResolvedValue(minimalTemplate);
  ...
  describe('【异常流】人为制造报错 → 精准捕获并抛出规范错误码', () => {
    ...
    it('TC-STG-ERR-002: template_id 缺失 → INVALID_REQUEST', async () => {
      const badRequest = { ...validRequest };
      delete badRequest.template_id;
    ...
    it('TC-STG-ERR-003: product_id 对应的商品不存在 → PRODUCT_NOT_FOUND', async () => {
    ...
    it('TC-STG-ERR-004: template_id 对应模板不存在 → TEMPLATE_NOT_FOUND', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(null);

      let caught: Error & { errorCode?: string; statusCode?: number } | null = null;
      try {
        await generateTemplate(
          { ...validRequest, template_id: '99999999-9999-9999-9999-999999999999' },
          {
            prisma: mockPrisma,
            doubao: mockDoubao,
            buildPrompt: buildTemplatePrompt,
    ...
    it('TC-STG-ERR-005: template status 为 INACTIVE → TEMPLATE_NOT_ACTIVE', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({ status: 'INACTIVE' }),
      );

      let caught: Error & { errorCode?: string; statusCode?: number } | null = null;
      try {
        await generateTemplate(validRequest, {
          prisma: mockPrisma,
          doubao: mockDoubao,
          buildPrompt: buildTemplatePrompt,
          parseResponse: parseScriptFromAIResponse,
      ...
      expect(caught!.statusCode).toBe(HttpStatus.BAD_REQUEST);
      expect(caught!.message).toContain('INACTIVE');
    });

    it('TC-STG-ERR-006: template status 为 ARCHIVED → TEMPLATE_NOT_ACTIVE', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
      ...
      expect(caught!.errorCode).toBe('TEMPLATE_NOT_ACTIVE');
      expect(caught!.statusCode).toBe(HttpStatus.BAD_REQUEST);
      expect(caught!.message).toContain('ARCHIVED');
    });

    it('TC-STG-ERR-007: template factorJson 为空对象 → TEMPLATE_FACTOR_EMPTY', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({ factor_json: {} }),
      );
      ...
      expect(caught).not.toBeNull();
      expect(caught!.errorCode).toBe('TEMPLATE_FACTOR_EMPTY');
      expect(caught!.statusCode).toBe(HttpStatus.BAD_REQUEST);
      expect(caught!.message).toContain('因子配置为空');
    });

    it('TC-STG-ERR-008: template strategySummary 为空字符串 → TEMPLATE_FACTOR_EMPTY', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({
          strategy_summary: '',
          factor_json: { optimal_shot_count: 4 },
      ...
      expect(caught!.errorCode).toBe('TEMPLATE_FACTOR_EMPTY');
      expect(caught!.statusCode).toBe(HttpStatus.BAD_REQUEST);
      expect(caught!.message).toContain('策略摘要为空');
    });

    // ---- 3.3 AI 调用层异常 ----

    it('TC-STG-ERR-009: AI 返回空字符串 → MODEL_PROVIDER_FAILED', async () => {
    ...
    it('TC-STG-ERR-010: AI 返回不可解析的非 JSON 文本 → SCRIPT_PARSE_FAILED', async () => {
    ...
    it('TC-STG-ERR-011: AI 返回合法 JSON 但缺少 shots 字段 → SCRIPT_PARSE_FAILED', async () => {
      mockDoubao.generateText.mockResolvedValue(
    ...
    it('TC-STG-ERR-012: AI 返回空 shots 数组 → SCRIPT_NO_SHOTS_GENERATED', async () => {
      mockDoubao.generateText.mockResolvedValue(
    ...
    it('TC-STG-ERR-013: 总时长 > 15.0s → SCRIPT_DURATION_EXCEEDED', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
          title: 'Over Duration',
          video_duration: 18.0,
    ...
    it('TC-STG-ERR-014: 单分镜时长 < 1.5s → SCRIPT_SCHEMA_INVALID', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
    ...
    it('TC-STG-ERR-015: 单分镜时长 > 5.0s → SCRIPT_SCHEMA_INVALID', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
          title: 'Too Long Shot',
          video_duration: 7.0,
    ...
    it('TC-STG-ERR-019: 含"最好"绝对化用语 → COMPLIANCE_CHECK_FAILED', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
    ...
    it('TC-STG-ERR-020: 含"免费送"禁止性促销表达 → COMPLIANCE_CHECK_FAILED', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
    ...
    it('TC-STG-ERR-021: 含"第一"绝对化用语 → COMPLIANCE_CHECK_FAILED', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
    ...
    it('TC-STG-ERR-022: 字幕中含"马上抢"禁止性表达 → COMPLIANCE_CHECK_FAILED', async () => {
      mockDoubao.generateText.mockResolvedValue(
        JSON.stringify({
    ...
    it('TC-STG-ERR-023: 合规校验跨分镜检测多词违规', () => {
    ...
    it('TC-STG-ERR-024: Prisma $transaction 写入失败 → INTERNAL_SERVER_ERROR', async () => {
      ...
      faultyPrisma.template.findUnique.mockResolvedValue(mockTemplateFactory());
  ...
  describe('【性能流】耗时与资源卡点告警', () => {
    ...
    it('TC-STG-PERF-001: generateTemplate 编排总耗时 ≤ 5000ms (含 mock AI 响应)', async () => {
    ...
    it('TC-STG-PERF-002: parseScriptFromAIResponse ≤ 50ms (JSON 解析)', async () => {
      const PERF_CEILING_MS = 50;

      const start = performance.now();

      const result = parseScriptFromAIResponse(mockTemplateAIValidResponse, {});
    ...
    it('TC-STG-PERF-003: validateScriptSchema ≤ 10ms (5 分镜)', () => {
      const PERF_CEILING_MS = 10;
      const parsed = parseScriptFromAIResponse(mockTemplateAIValidResponse, {});
    ...
    it('TC-STG-PERF-004: checkCompliance ≤ 5ms (5 分镜 × 12 规则)', () => {
    ...
    it('TC-STG-PERF-005: buildTemplatePrompt ≤ 3ms', () => {
      ...
      const result = buildTemplatePrompt({
        ...
        aspect_ratio: '9:16',
        language: 'zh-CN',
        strategy_summary: mockTemplateFactory().strategy_summary,
        factor_json: mockTemplateFactory().factor_json,
      });
    ...
    it('TC-STG-PERF-006: 连续 10 次 generateTemplate 无内存泄漏表现', async () => {
      const ITERATIONS = 10;
    ...
    it('TC-STG-PERF-007: 校验大批量分镜 (20 分镜) ≤ 30ms 不退化', () => {
  ...
  describe('【原子函数】独立校验 buildTemplatePrompt / validateTemplate / parseScriptFromAIResponse / validateScriptSchema / checkCompliance', () => {
    it('buildTemplatePrompt 在 language 缺失时默认 zh-CN', () => {
      const result = buildTemplatePrompt({
        product_id: 'test',
        selling_points: ['test'],
        style_vibe: 'minimal',
        aspect_ratio: '9:16',
        strategy_summary: '测试策略',
        factor_json: { optimal_shot_count: 4 },
      });
    ...
    it('buildTemplatePrompt 注入 strategy_summary 到 systemPrompt', () => {
      const result = buildTemplatePrompt({
        product_id: 'test',
        selling_points: ['test'],
        style_vibe: 'minimal',
        aspect_ratio: '9:16',
        strategy_summary: '前3秒强视觉对比',
        factor_json: { optimal_shot_count: 5 },
      });

      expect(result.systemPrompt).toContain('前3秒强视觉对比');
    });

    it('buildTemplatePrompt 注入 factorJson 到 systemPrompt', () => {
      const result = buildTemplatePrompt({
        product_id: 'test',
        selling_points: ['test'],
        style_vibe: 'minimal',
        aspect_ratio: '9:16',
        strategy_summary: '测试策略',
        factor_json: { optimal_shot_count: 5, bgm_style: 'upbeat' },
      });

      expect(result.systemPrompt).toContain('optimal_shot_count');
      expect(result.systemPrompt).toContain('bgm_style');
    });

    it('buildTemplatePrompt 注入 target_audience', () => {
      const result = buildTemplatePrompt({
        product_id: 'test',
        selling_points: ['test'],
        style_vibe: 'minimal',
    ...
    it('buildTemplatePrompt 注入 constraint_list', () => {
      const result = buildTemplatePrompt({
        product_id: 'test',
        selling_points: ['test'],
        style_vibe: 'minimal',
    ...
    it('validateTemplate 正常模板通过校验', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(mockTemplateFactory());

      const template = await validateTemplate(mockTemplateFactory().id, mockPrisma);

      expect(template).toBeDefined();
      expect(template.id).toBe(mockTemplateFactory().id);
    ...
    it('validateTemplate 模板不存在 → TEMPLATE_NOT_FOUND', async () => {
      ...
      let caught: Error & { errorCode?: string; statusCode?: number } | null = null;
      try {
        await validateTemplate('nonexistent-id', mockPrisma);
      } catch (e) {
        caught = e as Error & { errorCode?: string; statusCode?: number };
      }
    ...
    it('validateTemplate 非 ACTIVATED → TEMPLATE_NOT_ACTIVE', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({ status: 'INACTIVE' }),
      );
      ...
      let caught: Error & { errorCode?: string; statusCode?: number } | null = null;
      try {
        await validateTemplate(mockTemplateFactory().id, mockPrisma);
      } catch (e) {
        caught = e as Error & { errorCode?: string; statusCode?: number };
      }
    ...
    it('validateTemplate factorJson 为空 → TEMPLATE_FACTOR_EMPTY', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({ factor_json: {} }),
      );
    ...
    it('validateTemplate strategySummary 为空 → TEMPLATE_FACTOR_EMPTY', async () => {
      mockPrisma.template.findUnique.mockResolvedValue(
        mockTemplateFactory({
          strategy_summary: '',
          factor_json: { optimal_shot_count: 4 },
...
// =============================================================================
// 测试基座文件版本标识
// 用途: 当项目进入 CI/CD 流水线后, 此文件用于断言模板驱动剧本生成功能
// 的完整性与正确性。待源码实现后移除 .skip 或直接运行。
//
// 用例编号映射:
//   TC-STG-001 ~ TC-STG-001-EXT      正常流 (Happy Path)
//   TC-STG-BND-001 ~ TC-STG-BND-010  边界流 (Edge Cases)
//   TC-STG-ERR-001 ~ TC-STG-ERR-024  异常流 (Error Flow)
//   TC-STG-PERF-001 ~ TC-STG-PERF-007 性能流 (Performance)
//
// 覆盖率维度:
//   ├── buildTemplatePrompt      (5 原子测试)
//   ├── validateTemplate         (5 原子测试)
//   ├── parseScriptFromAIResponse (2 原子测试)
//   ├── validateScriptSchema     (3 原子测试 + 集成覆盖)
//   ├── checkCompliance          (2 原子测试 + 集成覆盖)
//   └── generateTemplate         (10 集成测试)
//
// 总测试用例数: 63
// =============================================================================